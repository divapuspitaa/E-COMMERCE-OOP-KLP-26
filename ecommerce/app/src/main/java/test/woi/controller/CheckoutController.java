...
public class CheckoutController {
    ...
    public void initialize() {
        // Pastikan ToggleGroup ter-assign ke semua radio button
        // (fx:define injection kadang tidak reliable; inisialisasi manual lebih aman)
        if (paymentGroup == null) {
            paymentGroup = new javafx.scene.control.ToggleGroup();
        }
        rbSaldo.setToggleGroup(paymentGroup);
        rbTransfer.setToggleGroup(paymentGroup);
        rbCOD.setToggleGroup(paymentGroup);

        // Set userData jika belum di-set dari FXML
        if (rbSaldo.getUserData() == null)    rbSaldo.setUserData("SALDO");
        if (rbTransfer.getUserData() == null) rbTransfer.setUserData("TRANSFER");
        if (rbCOD.getUserData() == null)      rbCOD.setUserData("COD");
